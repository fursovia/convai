...
import os
from model.utils import decode, decode_rawtext
import pandas as pd
...
def input_fn_rawtext(data_dir, params, file_name, train_time=True, evaluate_epochs=None):
    dataset = tf.data.TFRecordDataset(os.path.join(data_dir, '{}.tfrecords'.format(file_name)))
    ...
    dataset = dataset.apply(tf.contrib.data.map_and_batch(decode_rawtext, params.batch_size, num_parallel_batches=4))
    dataset = dataset.prefetch(buffer_size=None)
    return dataset
